...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ page import="com.legou.modules.tradeorder.entity.Tradeorder" %>
<%@ page import="com.legou.modules.sys.entity.Admin" %>
<%
	String path = request.getContextPath();
	ArrayList<Tradeorder> tradeorderlist = new ArrayList<Tradeorder>();
	tradeorderlist = (ArrayList<Tradeorder>)session.getAttribute("tradeorderlist");
...
<html>
	<head>
		...
		<link rel="stylesheet" href="admincss/dingdan_management.css" />
	...
	<body>
		...
		<div class="main-container" id="main-container">
									...
									<div class="col-xs-12">
										<div class="contenter-wrap">
											<div class="add-button">
												<div class="search">
													<form action="<%=path%>/likedingdan" method="post" name="likefrm">
											...
											<div class="table-responsive">
												<table id="sample-table-1" class="table table-striped table-hover ">
													...
													<tbody>
													<input type="hidden" id="item_nums" value="<%=tradeorderlist.size() %>">
													<%
														for (int i=0; i<tradeorderlist.size(); i++){
															%>
															<tr id="tr<%=i %>">
															<td><%=i+1 %></td>
															<td><%=tradeorderlist.get(i).getDingdanhao() %></td>
															<td><%=tradeorderlist.get(i).getShouhuoren() %></td>
															<td><%=tradeorderlist.get(i).getSpId() %></td>
															<td><%=tradeorderlist.get(i).getDizhi() %></td>
															<td><%=tradeorderlist.get(i).getYoubian() %></td>
															<td><%=tradeorderlist.get(i).getTel() %></td>
															<td>
																<div class="user-type">
																	<%=tradeorderlist.get(i).getZt().equals("1")?"已收货":"未收货" %>
																</div>
															</td>
															<td>
																<div class="button">
																	<button class="btton look-btn" data-toggle="modal" data-target="#myModal" 
																	id=<%=tradeorderlist.get(i).getId() %> dingdanhao=<%=tradeorderlist.get(i).getDingdanhao() %> 
																	shouhuoren=<%=tradeorderlist.get(i).getShouhuoren() %> dizhi=<%=tradeorderlist.get(i).getDizhi() %> youbian="<%=tradeorderlist.get(i).getYoubian() %>" 
																	tel="<%=tradeorderlist.get(i).getTel() %>" shsi="<%=tradeorderlist.get(i).getShsi() %>" 
																	zffs="<%=tradeorderlist.get(i).getZffs() %>" leaveword="<%=tradeorderlist.get(i).getLeaveword() %>" 
																	time="<%=tradeorderlist.get(i).getTime() %>" onclick="editInfo(this)">查看</button>
																	<button class="btton remove-btn" id="del_btn" userId=<%=tradeorderlist.get(i).getId() %> onclick="delUser(this)">删除</button>
																</div>
															</td>
														</tr>
															<%
			...
			<!-- /.modal -->
											...
											<form action="<%=path %>/deldingdan" name="delfrm" method="post">
											...
											<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
												<div class="modal-dialog">
													<div class="modal-content">
														...
														<div class="modal-body">
															<form role="form" name="frm2" action="<%=path %>/uptradeorder" method="post">
																<div class="from-group">
																	<label class="control-label"><h5>订单号</h5></label>
																	<input type="hidden" name="up_typend_id" id="up_typend_id">
																	<input class="form-control" name="dingdanhao" id="dingdanhao" disabled="disabled">
																</div>
																<div class="from-group">
																	<label class="control-label"><h5>收货人</h5></label>
																	<input class="form-control " name="shouhuoren" id="shouhuoren">
																</div>
																<div class="from-group">
																	<label class="control-label"><h5>收货地址</h5></label>
																	<input class="form-control " name="dizhi" id="dizhi">
																</div>
																<div class="from-group">
																	<label class="control-label"><h5>邮编</h5></label>
																	<input class="form-control " name="youbian" id="youbian">
																</div>
																<div class="from-group">
																...
																<div class="from-group">
																	<label class="control-label"><h5>收货时间</h5></label>
																	<input class="form-control " name="shsi" id="shsi" disabled="disabled">
																</div>
																<div class="from-group">
																	<label class="control-label"><h5>支付方式</h5></label>
																	<input class="form-control " name="zffs" id="zffs" disabled="disabled">
																</div>
																<div class="from-group">
																	<label class="control-label"><h5>买家留言</h5></label>
																	<input class="form-control" name="leaveword" id="leaveword" disabled="disabled">
																</div>
																<div class="from-group">
																	<label class="control-label"><h5>下单时间</h5></label>
																	<input class="form-control" name="time" id="time" disabled="disabled">
																</div>
...
<![endif]-->
	...
	<script type="text/javascript" src="adminjs/dingdan_management.js"></script> 
	<script type="text/javascript" src="adminjs/jquery-2.2.4.min.js"></script> 
</html>
<script>
function editInfo(obj) {  
    var id = $(obj).attr("id");
    var dingdanhao = $(obj).attr("dingdanhao");
    var shouhuoren = $(obj).attr("shouhuoren");
    var dizhi = $(obj).attr("dizhi");
    var youbian = $(obj).attr("youbian");
    var tel = $(obj).attr("tel");
    var shsi = $(obj).attr("shsi");
    var zffs = $(obj).attr("zffs");
    var leaveword = $(obj).attr("leaveword");
    var time = $(obj).attr("time");
    //向模态框中传值  
    $('#up_typend_id').val(id); 
    $('#dingdanhao').val(dingdanhao); 
    $('#shouhuoren').val(shouhuoren); 
    $('#dizhi').val(dizhi); 
    $('#youbian').val(youbian); 
    $('#tel').val(tel); 
    $('#shsi').val(shsi); 
...
//提交更改  
function update() { 
	if($("input[name=shouhuoren]").val() == null || $("input[name=shouhuoren]").val() == ""){
		swal({
			title:"收货人不能为空，请重填！",
			type:"error",
		...
		return;
	} else if($("input[name=dizhi]").val() == null || $("input[name=dizhi]").val() == ""){
		swal({
			title:"收货地址不能为空，请重填！",
			type:"error",
			confirmButtonText:"ok",
		...
		return;
	}else if($("input[name=youbian]").val() == null || $("input[name=youbian]").val() == ""){
		swal({
			title:"邮编不能为空，请重填！",
			type:"error",
			confirmButtonText:"ok",
		...
		return;
	} else if(!$("input[name=youbian]").val().match(/^[1-9][0-9]{5}$/)){
		swal({
			title:"邮编格式不正确，请重填！",
			type:"error",
			confirmButtonText:"ok",
		...
		return;
	} else if(!$("input[name=tel]").val().match(/^(13[0-9]{9})|(15[89][0-9]{8})$/)){
		swal({
			title:"手机格式不正确，请重填！",
			type:"error",
			confirmButtonText:"ok",
