<%@page import="com.legou.modules.message.entity.Leaveword"%>
<%@page import="com.legou.modules.message.entity.Huifu"%>
...
<%
	...
	huifulist = (ArrayList<Huifu>)session.getAttribute("huifulist");
	ArrayList<Leaveword> leavewordlist = new ArrayList<Leaveword>();
	leavewordlist = (ArrayList<Leaveword>)session.getAttribute("leavewordlist");
...
<html>
	<head>
		...
		<link rel="stylesheet" href="admincss/huifu_management.css" />
	...
	<body>
		...
		<div class="main-container" id="main-container">
									...
									<div class="col-xs-12">
										<div class="contenter-wrap">
											<div class="add-button">
												<div class="search">
												<form action="<%=path%>/likehuifu" method="post" name="likefrm">
											...
											<div class="table-responsive">
												<table id="sample-table-1" class="table table-striped table-hover ">
													...
													<tbody>
													<input type="hidden" id="item_nums" value="<%=huifulist.size() %>">
													<%
														for (int i=huifulist.size()-1; i>=0; i--){
															%>
															<tr id="tr<%=i %>">
															<td><%=huifulist.size()-i %></td>
															<%
																for (int j=0; j<userlist.size(); j++){
																	if (userlist.get(j).getId() == huifulist.get(i).getUserId()){
																		%>
																		<td><%=userlist.get(j).getName() %></td>
																		<input type="hidden" id="lyuser" name="lyuser" value="<%=userlist.get(j).getName()%>">
																		<%
																	}
																}
															%>
															<%
																for (int k=0; k<leavewordlist.size(); k++){
																	if (leavewordlist.get(k).getId() == huifulist.get(i).getLeavewordId()){
																		%>
																		<td><%=leavewordlist.get(k).getContent() %></td>
																		<input type="hidden" id="lycon" name="lycon" value="<%=leavewordlist.get(k).getContent()%>">
																		<%
																	}
																}
															%>
															<td><%=huifulist.get(i).getTitle() %></td>
															<td><%=huifulist.get(i).getContent() %></td>
															<td><%=huifulist.get(i).getTime() %></td>
															<td>
																<div class="user-type">
																	<%=huifulist.get(i).getYidu()==0?"未读":"已读" %>
																	
																</div>
															</td>
															<td>
																<div class="button">
																	<button class="btton look-btn" data-toggle="modal" data-target="#myModal" id="<%=huifulist.get(i).getLeavewordId() %>" username="<%=huifulist.get(i).getUserId() %>" title="<%=huifulist.get(i).getTitle() %>" content="<%=huifulist.get(i).getContent() %>" onclick="editInfo(this)">回复</button>
																	<button class="btton remove-btn" id="del_btn" userId=<%=huifulist.get(i).getId() %> onclick="delUser(this)">删除</button>
																</div>
															</td>
														</tr>
															<%
			...
			<!-- /.modal -->
											...
											<form action="<%=path %>/delhuifu" name="delfrm" method="post">
											...
											<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
												<div class="modal-dialog">
													<div class="modal-content">
														...
														<div class="modal-body">
															<form role="form" name="addfrm" action="<%=path%>/addhuifu" method="post">
																<input type="hidden" name="huifuid2" id="huifuid2" value="">
																<div class="from-group">
																	<label class="control-label"><h5>To:<input type="hidden" name="hfusername" id="hfusername" value=""><span id="lyu"></span></h5></label>
																</div>
																<div class="from-group">
																	<label class="control-label"><h5>回复主题</h5></label>
																	<textarea class="form-control" rows="0" cols="2" name="hftitle" id="hftitle"> </textarea>
																</div>
																<div class="from-group">
																	<label class="control-label"><h5>回复内容</h5></label>
																	<textarea class="form-control" rows="0" cols="2" name="hfcontent" id="hfcontent"> </textarea>
																</div>	
															</form>
														</div>
														<div class="modal-footer">
															<button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
															<button type="button" class="btn btn-xiugai" data-dismiss="modal" onclick="addItem()">回复</button>
														</div>
													</div>
													<!-- /.modal-content -->
												</div>
...
<![endif]-->
	...
	<script type="text/javascript" src="adminjs/huifu_management.js"></script> 
...
function editInfo(obj) {  
    var id = $(obj).attr("id");
    var lyuser = $('#lyuser').val();
    var username = $(obj).attr("username");
 
    //向模态框中传值  
    $("#huifuid2").val(id);
    $("#hfusername").val(username);
    var t = document.getElementById('lyu');
    t.innerHTML=lyuser;
} 
