									...
									<div class="col-xs-12">
										<div class="contenter-wrap">
											<div class="add-button">
												...
												<div class="search">
													<form action="<%=path%>/liketyperd" method="post" name="likefrm">
											...
											<div class="table-responsive">
												<table id="sample-table-1" class="table table-striped table-hover ">
													...
													<tbody>
													<input type="hidden" id="item_nums" value="<%=categorythirdlist.size() %>">
													<%
														for (int i=0; i<categorythirdlist.size(); i++){
															%>
															<tr id="tr<%=i %>">
															<td><%=i+1 %></td>
															<td><%=categorythirdlist.get(i).getTypeId() %></td>
															<td><%=categorythirdlist.get(i).getTypename() %></td>
															<td>
																<div class="button">
																	<button class="btton look-btn" data-toggle="modal" data-target="#myModal" id=<%=categorythirdlist.get(i).getId() %> typename="<%=categorythirdlist.get(i).getTypename() %>" typeid=<%=categorythirdlist.get(i).getTypeId() %>  onclick="editInfo(this)">查看</button>
																	<button class="btton remove-btn" id="del_btn" userId=<%=categorythirdlist.get(i).getId() %> onclick="delUser(this)">删除</button>
																</div>
															</td>
														</tr>
															<%
											...
											<form action="<%=path %>/deltyperd" name="delfrm" method="post">
												...
												<div class="modal-dialog">
													<div class="modal-content">
														<div class="modal-header">
															...
															<h4 class="modal-title" id="myModalLabel">商品三级信息</h4>
														</div>
														<div class="modal-body">
															<form role="form" name="frm2" action="<%=path %>/uptyperd" method="post">
																<div class="from-group">
																	<label class="control-label"><h5>商品三级别名</h5></label>
																	<input type="hidden" name="up_typend_id" id="up_typend_id">
																	<input class="form-control " name="up_typename" id="up_typename">
																</div>
																<div class="from-group">
																	...
																	<%
																		for (int i=0; i<categorysecondlist.size(); i++){
																			%>
																			<option id="type_op<%=categorysecondlist.get(i).getId()%>" value="<%=categorysecondlist.get(i).getId()%>"><%=categorysecondlist.get(i).getTypename() %></option>
																			<%
																		}
																	%>
											...
											<div class="modal fade" id="add" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
												<div class="modal-dialog">
													<div class="modal-content">
														<div class="modal-header">
															<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
															<h4 class="modal-title" id="myModalLabel">商品三级添加</h4>
														</div>
														<div class="modal-body">
															<form role="form" name="addfrm" action="<%=path%>/addtyperd" method="post">
																<div class="from-group">
																	<label class="control-label"><h5>商品三级别名</h5></label>
																...
																<div class="from-group">
																	<label class="control-label"><h5>商品二级别名</h5></label>
																	<select id="add_typeid" name="add_typeid">
																		<%
																			for (int i=0; i<categorysecondlist.size(); i++){
																				%>
																				<option value="<%=categorysecondlist.get(i).getId()%>"><%=categorysecondlist.get(i).getTypename() %></option>
																				<%
																			}
																		%>
																	</select>
